...
#' # 0. Funkcja do obliczania czêstoœci wystêpowania s³ów
# 0. Funkcja do obliczania czêstoœci wystêpowania s³ów ----
...
#' # 0. Funkcja do tworzenia chmury s³ów 
# 0. Funkcja do tworzenia chmury s³ów ----
...
#' - normalizacja (ujednolicenie) wielkoœci liter (zamiana na ma³e litery = lowercase)
#' - normalizacja (ujednolicenie) rozbie¿nych kodowañ znaków (apostrofy, cudzys³owy)
#' - normalizacja (ujednolicenie) form skróconych (I'm, I've, don't) przez usuniêcie lub rozwiniêcie
#' - normalizacja (ujednolicenie) ró¿nych akcentów ("café" na "cafe") przez usuniêcie akcentów
#' - normalizacja (ujednolicenie) popularnych skrótów ("btw" na "by the way", "b4" na "before") przez rozwiniêcie
#' - usuniêcie zbêdnych ci¹gów znaków (adresy URL, tagi HTML)
#' - usuniêcie zbêdnych znaków specjalnych (*, &, #, @, $)
#' - usuniêcie zbêdnych bia³ych znaków (spacja, tabulacja, znak przejœcia do nowej linii "enter")
#' - usuniêcie cyfr i liczb
#' - usuniêcie interpunkcji
#' - tokenizacja (podzia³ tekstu na s³owa = tokeny)
#' - usuniêcie stopwords (s³ów o ma³ej wartoœci semantycznej, np. "the", "and")
#' - usuniêcie pustych elementów (rozwa¿enie problemu brakuj¹cych/niekompletnych danych )
#' - stemming lub lematyzacja (sprowadzenie s³ów do ich rdzenia/formy podstawowej)
#'


#' #### ???? Zliczanie czêstoœci s³ów <br>*(Word Frequency Count)*
#'

#' #### ???? Eksploracyjna analiza danych: <br>wizualizacja czêstoœci s³ów (tabela, wykres, chmura s³ów) <br>*(Exploratory Data Analysis, EDA)*
#'

#' #### ???? In¿ynieria cech w modelu Bag of Words: <br>reprezentacja tekstu jako zbioru s³ów i czêstoœci s³ów ( = cechy) <br>*(Feature Engineering in BoW model)*
#'



# Wczytanie i przetworzenie tekstu ----
# file_path <- "sciezka/do/pliku.txt"  <= Uzupe³nij nazwê pliku i ustaw Working Directory!
file_path <- "Trump.txt"
...
# Liczba s³ów drastycznie siê zmniejszy³a,
# poniewa¿ inner_join zachowa³ tylko te s³owa,
# które wystêpowa³y w s³owniku
...
# Zliczanie, które s³owa s¹ najczêstsze
...
# filtrowanie analizy sentymentu
# i pozostawienie tylko s³ów o wartoœci w zakresie od -5 do 5
...
#' # Analiza sentymentu w czasie o ustalonej d³ugoœci linii
# Analiza sentymentu w czasie o ustalonej d³ugoœci linii ----



# Po³¹czenie wszystkich s³ów words w jeden ci¹g znaków
